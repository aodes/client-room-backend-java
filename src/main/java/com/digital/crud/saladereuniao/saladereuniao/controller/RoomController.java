...
public class RoomController {
    ...
    @PutMapping("/rooms/{id}")
    public ResponseEntity<Room> updateRoom(@PathVariable(value = "id") Long roomId, @RequestBody @Valid Room newRoom) throws ResourceNotFoundException {

        Room room = roomRepository.findById(roomId).orElseThrow(() -> new ResourceNotFoundException("Room not found this id " + roomId));
        room.setName(newRoom.getName());
        room.setData(newRoom.getData());
